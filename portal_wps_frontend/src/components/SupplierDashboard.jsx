...
const SupplierDashboard = ({ user, token }) => {
  ...
  // Função para calcular horário a partir de uma posição Y (top)
  const calculateTimeFromTop = (top) => {
    const hoursFromStart = top / HOUR_HEIGHT
    ...
    const finalHour = min >= 45 ? (hour + 1) % 24 : hour
    
    return `${finalHour.toString().padStart(2, '0')}:${roundedMin.toString().padStart(2, '0')}`
  }
                        ...
                        return (
                          <div
                            ...
                            className={`absolute left-1 right-1 transition-all duration-200 ${
                              isUnavailable
                                ? 'border-2 border-dashed border-red-400 bg-red-50/30 rounded'
                                : isClickable
                          ...
                          >
                            {isUnavailable && (
                              <div className="absolute inset-0 flex items-center justify-center opacity-60 pointer-events-none">
                                <span className="text-[10px] text-red-700 font-medium">Indisponível</span>
                              </div>
                            )}
                            {isClickable && (
                              <>
